...
| Niche model       | @williamsSimpleRulesYield2000                | structural         | |
...
2. collate/translate into `Julia`
    * *e.g.,* some models wil be in ESpeciesInteractionNetworks.jl (new EcoNet); I know (parts of) the transfer learning stuff is and the niche model
...
> Thinking very long term here and maybe a bit beyond the scope but also thinking about a multi- model approach? SO using one model to build an initial network but maybe a second one to constrain it a bit better. I blame this thought on the over-connected PFIM food webs...
