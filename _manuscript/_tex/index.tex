...
\hypersetup{
  ...
  pdfauthor={Tanya Strydom; Timothée Poisot; Andrew P. Beckerman},
...
species involved. These two models are equivalent to the Erdos-Renyi
(Erdős and Rényi 1959) and Configuration models (Newman 2010)
respectively (check that though).
...
\textbf{Resource models:} In the context of network generating models
this is perhaps the most well known family of models. Essentially these
models can be viewed as being based on the idea of resource partitioning
...
interactions are based on the `feeding niche' of a species. Broadly, all
species are randomly assigned a `feeding niche' and all species that
fall in this niche can be consumed by that species. Finally, the nested
hierarchy model (Cattin et al. 2004) \textbf{TODO}.
...
trait-based k-NN (\emph{e.g.,} Desjardins-Proulx et al. 2017)
...
be used to predict interactions. Strydom et al. (2022) presents a
specific use case that is based on transfer learning and the idea that
interactions are evolutionarily conserved and that we can use known
networks, and this evolutionary relationship to predict interactions for
a given species pool. \textbf{TODO} Log-ratio (Rohr et al. 2010)
...
(Jennifer A. Dunne et al. 2008), or a combination of published and grey
literature (\emph{e.g.,} Maiorano et al. 2020).
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0859}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1016}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.1172}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2422}}@{}}
\caption{Lets make a table that gives an overview of the different model
families that we will look at. Here I take `data-driven' to refer to the
need for `real world' data. This can probably be approached in a
different way though maybe?}\label{tbl-history}\tabularnewline
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
null & Interactions occur randomly & network & link & binary & no \\
neutral & Interactions are `random' but species abundance plays a role &
network & & & abundance \\
resource & Interactions are resource constrained (trophic pyramid) &
network & link & binary & no \\
generative & Interactions are determined by structural features of the
network (\emph{pending}) & network & & & \\
energetic & Interactions are determined by foraging theory (feeding
links) & & & binary & yes (body size, \ldots) \\
graph embedding & Interactions can be predicted from the latent traits
of networks & interactions & & probabilistic & yes (network) \\
trait matching & Interactions can be inferred by a mechanistic framework
& interactions & & & yes (expert knowledge, trait) \\
binary classifiers & Interactions can be predicted by learning the
relationship between interactions and ecologically relevant &
interactions & & probabilistic & yes (interaction, trait) \\
\end{longtable}
